...
HAL_StatusTypeDef HAL_ETH_Init(ETH_HandleTypeDef *heth) {
  ...
  READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_SR);
  ...
  SET_BIT(heth->Instance->MMCRIMR,
          ETH_MMCRIMR_RGUFM | ETH_MMCRIMR_RFAEM | ETH_MMCRIMR_RFCEM);
...
HAL_StatusTypeDef HAL_ETH_Start_IT(ETH_HandleTypeDef *heth) {
  ...
  if (heth->gState == HAL_ETH_STATE_READY) {
    ...
    - Fatal bus interrupt
    */
    __HAL_ETH_DMA_ENABLE_IT(heth, (ETH_DMAIER_NISE | ETH_DMAIER_RIE |
                                   ETH_DMAIER_TIE | ETH_DMAIER_FBEIE |
...
HAL_StatusTypeDef HAL_ETH_Stop_IT(ETH_HandleTypeDef *heth) {
  ...
  if (heth->gState == HAL_ETH_STATE_STARTED) {
    ...
    __HAL_ETH_DMA_DISABLE_IT(heth, (ETH_DMAIER_NISE | ETH_DMAIER_RIE |
                                    ETH_DMAIER_TIE | ETH_DMAIER_FBEIE |
                                    ETH_DMAIER_AISE | ETH_DMAIER_RBUIE));
...
HAL_StatusTypeDef HAL_ETH_ReadData(ETH_HandleTypeDef *heth, void **pAppBuff) {
  ...
  while ((READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_OWN) == (uint32_t)RESET) &&
    ...
    if ((READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_FS) != (uint32_t)RESET) ||
      ...
      bufflength = ((dmarxdesc->DESC0 & ETH_DMARXDESC_FL) >>
...
static void ETH_UpdateDescriptor(ETH_HandleTypeDef *heth) {
    ...
    if (allocStatus != 0U) {
      if (heth->RxDescList.ItMode == 0U) {
        WRITE_REG(dmarxdesc->DESC1,
                  heth->Init.RxBuffLen | ETH_DMARXDESC_DIC | ETH_DMARXDESC_RCH);
      } else {
  ...
  if (heth->RxDescList.RxBuildDescCnt != desccount) {
    ...
    WRITE_REG(heth->Instance->DMARPDR,
...
HAL_StatusTypeDef HAL_ETH_GetRxDataErrorCode(const ETH_HandleTypeDef *heth,
  ...
  /* Get error bits. */
  *pErrorCode =
      READ_BIT(heth->RxDescList.pRxLastRxDesc, ETH_DMARXDESC_ERRORS_MASK);
...
HAL_StatusTypeDef HAL_ETH_PTP_GetConfig(ETH_HandleTypeDef *heth,
  ...
  ptpconfig->TimestampUpdate =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSFCU) >>
        ETH_PTPTSCR_TSFCU_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampAll =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSARFE) >>
        ETH_PTPTSCR_TSSARFE_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampRolloverMode =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSSR) >>
        ETH_PTPTSCR_TSSSR_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampV2 =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSPTPPSV2E) >>
        ETH_PTPTSCR_TSPTPPSV2E_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampEthernet =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSPTPOEFE) >>
        ETH_PTPTSCR_TSSPTPOEFE_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampIPv6 =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSIPV6FE) >>
        ETH_PTPTSCR_TSSIPV6FE_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampIPv4 =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSIPV4FE) >>
        ETH_PTPTSCR_TSSIPV4FE_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampEvent =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSEME) >>
        ETH_PTPTSCR_TSSEME_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampMaster =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSMRME) >>
        ETH_PTPTSCR_TSSMRME_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampFilter =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSPFFMAE) >>
        ETH_PTPTSCR_TSPFFMAE_Pos) > 0U)
          ? ENABLE
          : DISABLE;
  ptpconfig->TimestampClockType =
      ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSCNT) >>
...
HAL_StatusTypeDef HAL_ETH_PTP_AddTimeOffset(ETH_HandleTypeDef *heth,
  ...
  if (heth->IsPtpConfigured == HAL_ETH_PTP_CONFIGURED) {
    if (ptpoffsettype == HAL_ETH_PTP_NEGATIVE_UPDATE) {
      ...
      if (READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSSR) ==
          ETH_PTPTSCR_TSSSR) {
        /* Set nanoSeconds update */
        heth->Instance->PTPTSLUR = ETH_PTPTSLR_VALUE - timeoffset->NanoSeconds;
      } else {
        heth->Instance->PTPTSLUR =
            ETH_PTPTSHR_VALUE - timeoffset->NanoSeconds + 1U;
      }
...
void HAL_ETH_IRQHandler(ETH_HandleTypeDef *heth) {
  ...
  /* Packet received */
  if (((dma_flag & ETH_DMASR_RS) != 0U) &&
      ((dma_itsource & ETH_DMAIER_RIE) != 0U)) {
  ...
  /* Packet transmitted */
  if (((dma_flag & ETH_DMASR_TS) != 0U) &&
      ((dma_itsource & ETH_DMAIER_TIE) != 0U)) {
  ...
  /* ETH DMA Error */
  if (((dma_flag & ETH_DMASR_AIS) != 0U) &&
      ((dma_itsource & ETH_DMAIER_AISE) != 0U)) {
    heth->ErrorCode |= HAL_ETH_ERROR_DMA;
    ...
    if ((dma_flag & ETH_DMASR_FBES) != 0U) {
      ...
      heth->DMAErrorCode =
          READ_BIT(heth->Instance->DMASR, (ETH_DMASR_ETS | ETH_DMASR_RWTS |
  ...
  if ((mac_flag & ETH_MAC_PMT_IT) != 0U) {
    ...
    heth->MACWakeUpEvent = READ_BIT(heth->Instance->MACPMTCSR,
                                    (ETH_MACPMTCSR_WFR | ETH_MACPMTCSR_MPR));
...
HAL_StatusTypeDef HAL_ETH_ReadPHYRegister(ETH_HandleTypeDef *heth,
  ...
  tmpreg1 |=
      ((PHYAddr << 11U) & ETH_MACMIIAR_PA); /* Set the PHY device address   */
  tmpreg1 |= (((uint32_t)PHYReg << 6U) &
...
HAL_StatusTypeDef HAL_ETH_GetMACConfig(const ETH_HandleTypeDef *heth,
  ...
  macconf->RetryTransmission =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_RD) >> 9) == 0U) ? ENABLE
                                                                   : DISABLE;
  macconf->CarrierSenseDuringTransmit =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_CSD) >> 16) > 0U) ? ENABLE
                                                                    : DISABLE;
  macconf->ReceiveOwn =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_ROD) >> 13) == 0U) ? ENABLE
                                                                     : DISABLE;
  ...
  macconf->Jabber =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_JD) >> 22) == 0U) ? ENABLE
                                                                    : DISABLE;
  macconf->Watchdog =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_WD) >> 23) == 0U) ? ENABLE
                                                                    : DISABLE;
  macconf->AutomaticPadCRCStrip =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_APCS) >> 7) > 0U) ? ENABLE
                                                                    : DISABLE;
  macconf->InterPacketGapVal = READ_BIT(heth->Instance->MACCR, ETH_MACCR_IFG);
  macconf->ChecksumOffload =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_IPCO) >> 10U) > 0U) ? ENABLE
                                                                      : DISABLE;
  macconf->CRCStripTypePacket =
      ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_CSTF) >> 25U) > 0U) ? ENABLE
                                                                      : DISABLE;

  macconf->TransmitFlowControl =
      ((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_TFCE) >> 1) > 0U) ? ENABLE
                                                                      : DISABLE;
  macconf->ZeroQuantaPause =
      ((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_ZQPD) >> 7) == 0U)
          ? ENABLE
          : DISABLE;
  macconf->PauseLowThreshold = READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_PLT);
  macconf->PauseTime = (READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_PT) >> 16);
  macconf->ReceiveFlowControl =
      ((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_RFCE) >> 2U) > 0U)
          ? ENABLE
          : DISABLE;
  macconf->UnicastPausePacketDetect =
      ((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_UPFD) >> 3U) > 0U)
          ? ENABLE
          : DISABLE;
...
HAL_StatusTypeDef HAL_ETH_GetDMAConfig(const ETH_HandleTypeDef *heth,
  ...
  dmaconf->AddressAlignedBeats =
      ((READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_AAB) >> 25U) > 0U)
          ? ENABLE
          : DISABLE;
  dmaconf->BurstMode =
      READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_FB | ETH_DMABMR_MB);
  ...
  dmaconf->EnhancedDescriptorFormat =
      ((READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_EDE) >> 7) > 0U) ? ENABLE
                                                                     : DISABLE;
  dmaconf->DescriptorSkipLength =
      READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_DSL) >> 2;

  dmaconf->DropTCPIPChecksumErrorFrame =
      ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_DTCEFD) >> 26) > 0U)
          ? DISABLE
          : ENABLE;
  dmaconf->ReceiveStoreForward =
      ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_RSF) >> 25) > 0U) ? ENABLE
                                                                      : DISABLE;
  dmaconf->FlushRxPacket =
      ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_FTF) >> 20) > 0U) ? DISABLE
                                                                      : ENABLE;
  dmaconf->TransmitStoreForward =
      ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_TSF) >> 21) > 0U) ? ENABLE
                                                                      : DISABLE;
  dmaconf->TransmitThresholdControl =
      READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_TTC);
  dmaconf->ForwardErrorFrames =
      ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_FEF) >> 7) > 0U) ? ENABLE
                                                                     : DISABLE;
  dmaconf->ForwardUndersizedGoodFrames =
      ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_FUGF) >> 6) > 0U) ? ENABLE
                                                                      : DISABLE;
  dmaconf->ReceiveThresholdControl =
      READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_RTC);
  dmaconf->SecondFrameOperate =
      ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_OSF) >> 2) > 0U) ? ENABLE
                                                                     : DISABLE;
...
HAL_ETH_GetMACFilterConfig(const ETH_HandleTypeDef *heth,
  ...
  pFilterConfig->PromiscuousMode =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_PM)) > 0U) ? ENABLE
                                                               : DISABLE;
  pFilterConfig->HashUnicast =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HU) >> 1) > 0U) ? ENABLE
                                                                    : DISABLE;
  pFilterConfig->HashMulticast =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HM) >> 2) > 0U) ? ENABLE
                                                                    : DISABLE;
  pFilterConfig->DestAddrInverseFiltering =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_DAIF) >> 3) > 0U) ? ENABLE
                                                                      : DISABLE;
  pFilterConfig->PassAllMulticast =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_PAM) >> 4) > 0U) ? ENABLE
                                                                     : DISABLE;
  pFilterConfig->BroadcastFilter =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_BFD) >> 5) > 0U) ? ENABLE
                                                                     : DISABLE;
  pFilterConfig->ControlPacketsFilter =
      READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_PCF);
  pFilterConfig->SrcAddrInverseFiltering =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_SAIF) >> 8) > 0U) ? ENABLE
                                                                      : DISABLE;
  pFilterConfig->SrcAddrFiltering =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_SAF) >> 9) > 0U) ? ENABLE
                                                                     : DISABLE;
  pFilterConfig->HachOrPerfectFilter =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HPF) >> 10) > 0U) ? ENABLE
                                                                      : DISABLE;
  pFilterConfig->ReceiveAllMode =
      ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_RA) >> 31) > 0U) ? ENABLE
                                                                     : DISABLE;
...
void HAL_ETH_ExitPowerDownMode(ETH_HandleTypeDef *heth) {
  ...
  CLEAR_BIT(heth->Instance->MACPMTCSR,
            ETH_MACPMTCSR_WFE | ETH_MACPMTCSR_MPE | ETH_MACPMTCSR_GU);
...
static void ETH_SetMACConfig(ETH_HandleTypeDef *heth,
  ...
  tmpreg1 |= (uint32_t)(
      (macconf->PauseTime << 16U) |
      ((uint32_t)((macconf->ZeroQuantaPause == DISABLE) ? 1U : 0U) << 7U) |
      macconf->PauseLowThreshold |
      ((uint32_t)((macconf->UnicastPausePacketDetect == ENABLE) ? 1U : 0U)
       << 3U) |
...
static void ETH_SetDMAConfig(ETH_HandleTypeDef *heth,
  ...
  tmpreg1 |= (uint32_t)(
      ((uint32_t)((dmaconf->DropTCPIPChecksumErrorFrame == DISABLE) ? 1U : 0U)
       << 26U) |
  ...
  (heth->Instance)->DMABMR = (uint32_t)(
      ((uint32_t)dmaconf->AddressAlignedBeats << 25U) | dmaconf->BurstMode |
...
static void ETH_MACDMAConfig(ETH_HandleTypeDef *heth) {
  ...
  dmaDefaultConf.TransmitThresholdControl =
